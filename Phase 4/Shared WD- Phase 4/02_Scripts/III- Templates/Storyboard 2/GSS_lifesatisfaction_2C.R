# ==============================================================================
# SCRIPT: GSS_lifesatisfaction_2C.R
# Purpose: Generate 3-Country Skyline for Overall Life Satisfaction
...
prepared_data <- raw_data %>%
  ...
  mutate(
    ...
    Life_Satisfaction_Index  = if_else(`Personal Happiness` == 1 & `Financial Satisfaction` == 1 & `Life Engagement` == 1, 1, 0),
...
overall_tercile <- prepared_data %>%
  ...
  summarise(
    ...
    `Personal Happiness (%)`     = round((sum(`Personal Happiness` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Financial Satisfaction (%)` = round((sum(`Financial Satisfaction` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Life Engagement (%)`        = round((sum(`Life Engagement` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Satisfaction Index (%)`     = round((sum(Life_Satisfaction_Index * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    `Personal Happiness`     = weighted.mean(`Personal Happiness`, w = PERWT, na.rm = TRUE),
    `Financial Satisfaction` = weighted.mean(`Financial Satisfaction`, w = PERWT, na.rm = TRUE),
    `Life Engagement`        = weighted.mean(`Life Engagement`, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c("Personal Happiness", "Financial Satisfaction", "Life Engagement"), names_to = "Variable", values_to = "val") %>%
  mutate(Country = factor(Country, levels = c("Bottom Third", "Middle Third", "Top Third"))) %>%
...
viz_data_single <- prepared_data %>%
  group_by(Country) %>% mutate(decile = ntile(REAL_INCOME, 10)) %>% group_by(Country, decile) %>%
  summarise(avg_val = weighted.mean(Life_Satisfaction_Index, w = PERWT, na.rm = TRUE), .groups = "drop") %>%
...
ggsave(here::here("03_output", "visualizations_final", "GSS_life_satisfaction_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "GSS_life_satisfaction_pillars_2C.png"), p_pillars, width = 10, height = 7, dpi = 300, bg = "white")
ggsave(here::here("03_output", "visualizations_final", "GSS_life_satisfaction_composite_2C.png"), p_index, width = 10, height = 7, dpi = 300, bg = "white")
