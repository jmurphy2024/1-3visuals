# ==============================================================================
# SCRIPT: GSS_lifelong_learning_2C.R
...
prepared_data <- raw_data %>%
  ...
  mutate(
    ...
    Lifelong_Learning_Index   = if_else(`Knowledge Acquisition` == 1 & `Cognitive Maintenance` == 1 & `Foundational Competency` == 1, 1, 0),
...
overall_tercile <- prepared_data %>%
  ...
  summarise(
    ...
    `Knowledge Acquisition (%)`   = round((sum(`Knowledge Acquisition` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Cognitive Maintenance (%)`   = round((sum(`Cognitive Maintenance` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Foundational Competency (%)` = round((sum(`Foundational Competency` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Learning Index (%)`          = round((sum(Lifelong_Learning_Index * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    `Knowledge Acquisition`   = weighted.mean(`Knowledge Acquisition`, w = PERWT, na.rm = TRUE),
    `Cognitive Maintenance`   = weighted.mean(`Cognitive Maintenance`, w = PERWT, na.rm = TRUE),
    `Foundational Competency` = weighted.mean(`Foundational Competency`, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c("Knowledge Acquisition", "Cognitive Maintenance", "Foundational Competency"), names_to = "Variable", values_to = "val") %>%
  mutate(Country = factor(Country, levels = c("Bottom Third", "Middle Third", "Top Third"))) %>%
...
viz_data_single <- prepared_data %>%
  group_by(Country) %>% mutate(decile = ntile(REAL_INCOME, 10)) %>% group_by(Country, decile) %>%
  summarise(avg_val = weighted.mean(Lifelong_Learning_Index, w = PERWT, na.rm = TRUE), .groups = "drop") %>%
...
ggsave(here::here("03_output", "visualizations_final", "GSS_lifelong_learning_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "GSS_lifelong_learning_pillars_2C.png"), p_pillars, width = 10, height = 7, dpi = 300, bg = "white")
ggsave(here::here("03_output", "visualizations_final", "GSS_lifelong_learning_composite_2C.png"), p_index, width = 10, height = 7, dpi = 300, bg = "white")
