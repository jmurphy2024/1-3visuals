# ==============================================================================
# SCRIPT: GSS_socioeconomic_mobility_2C.R
...
prepared_data <- raw_data %>%
  ...
  mutate(
    ...
    Mobility_Index               = if_else(`Intergenerational Mobility` == 1 & `Intragenerational Mobility` == 1 & `Systemic Mobility` == 1, 1, 0),
...
# ------------------------------------------------------------------------------
message("\n=== SOCIOECONOMIC MOBILITY SUMMARY ===")
overall_tercile <- prepared_data %>%
  group_by(Country) %>%
  summarise(
    Subgroup = "Overall Tercile", Total_Population = sum(PERWT, na.rm = TRUE),
    `Intergenerational (%)` = round((sum(`Intergenerational Mobility` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Intragenerational (%)` = round((sum(`Intragenerational Mobility` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Systemic Mobility (%)` = round((sum(`Systemic Mobility` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    `Intergenerational Mobility` = weighted.mean(`Intergenerational Mobility`, w = PERWT, na.rm = TRUE),
    `Intragenerational Mobility` = weighted.mean(`Intragenerational Mobility`, w = PERWT, na.rm = TRUE),
    `Systemic Mobility`          = weighted.mean(`Systemic Mobility`, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c("Intergenerational Mobility", "Intragenerational Mobility", "Systemic Mobility"), names_to = "Variable", values_to = "val") %>%
  mutate(Country = factor(Country, levels = c("Bottom Third", "Middle Third", "Top Third"))) %>%
...
ggsave(here::here("03_output", "visualizations_final", "GSS_socioeconomic_mobility_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "GSS_socioeconomic_mobility_pillars_2C.png"), p_pillars, width = 10, height = 7, dpi = 300, bg = "white")
ggsave(here::here("03_output", "visualizations_final", "GSS_socioeconomic_mobility_composite_2C.png"), p_index, width = 10, height = 7, dpi = 300, bg = "white")
