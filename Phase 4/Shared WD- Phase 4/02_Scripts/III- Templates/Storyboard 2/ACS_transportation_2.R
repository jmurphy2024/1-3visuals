# ==============================================================================
# SCRIPT: ACS_vehicle_occupancy_2.R
# Purpose: Generate 3-Country Skyline for Vehicle Occupancy (Drove Alone vs Carpool)
...
# Target Variable: RIDERS (Vehicle occupancy for private vehicle commuters)
VARS_NEEDED <- c("SERIAL", "PERWT", "INCTOT", "STATEFIP", "AGE", "RIDERS")

TARGET_DIR  <- here::here("01_data", "raw", "IPUMS_Microdata", "acs_riders")
TARGET_FILE <- file.path(TARGET_DIR, "acs_riders_aggregated.rds")
...
if (!file.exists(TARGET_FILE)) {
  ...
  extract_def <- define_extract_micro(
    ...
    description = "Three Countries ACS Skyline - Vehicle Occupancy (V2)"
  )
...
prepared_data <- as_tibble(dt_raw) %>%
  ...
  mutate(
    ...
    riders_num = as.numeric(RIDERS),
    
    # --------------------------------------------------------------------------
    # INDICATOR LOGIC: Drove Alone vs. Carpool
    # --------------------------------------------------------------------------
    Drove_Alone = if_else(riders_num == 1, 1, 0),
    Carpool     = if_else(riders_num > 1, 1, 0),
  ...
  # UNIVERSE: Must be a private vehicle commuter (riders_num > 0 removes N/A, Walkers, Transit, WFH)
  filter(!is.na(PERWT), PERWT > 0, !is.na(REAL_INCOME), !is.na(Country), riders_num > 0)

# 5. VISUALIZATION
# ------------------------------------------------------------------------------
p <- plot_economic_skyline_2(
  data           = prepared_data, 
  indicator_vars = c("Drove_Alone", "Carpool"), 
  weight_var     = "PERWT", 
  y_axis_label   = "Private Vehicle Occupancy Rate (%)",
  plot_title     = "ACS_vehicle_occupancy_2"
) +
  labs(
    caption = stringr::str_wrap("Note: Universe restricted to workers commuting via private vehicle (auto, truck, or van). Income is generated by natively aggregating personal income (INCTOT) within households. Data is adjusted for inflation and spatial price parity.", width = 110)
  ) +
  theme(
