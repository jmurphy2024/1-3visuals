# ==============================================================================
# SCRIPT: NCVS_crime_rates_2.R
# Purpose: Generate 3-Country Multi-Variable Skyline for Victimization Rates
# Logic:   Dynamic RDA Load + 3-Way Join + Jittering + Dynamic V2 Borders
# ==============================================================================
rm(list = ls()); gc()
...
prepared_data <- ds3_unique %>%
  ...
  left_join(ds5_raw %>% select(IDHH, IDPER, V4529), 
            by = c("IDHH", "IDPER"), relationship = "one-to-many") %>%
  ...
  mutate(
    ...
    # Victimization Flags (Now named for clean multi-line labeling)
  ...
  summarise(
    ...
    Violent_Crime = max(Violent_Crime, na.rm = TRUE),
  ...
  mutate(
    ...
    # E. Dynamic V2 Country Assignment
    Country = case_when(
...
# 6. VISUALIZATION (Multi-Variable V2)
# ------------------------------------------------------------------------------
# Rename the columns to include spaces just for the final plot labels
...
p <- plot_economic_skyline_2(
  ...
  # Reference the new spaced names (Make sure to use quotes!)
  indicator_vars = c("Property Crime", "Violent Crime"), 
  
  weight_var     = "PERWT", 
  y_axis_label   = "Victimization Rate (%)",
  plot_title     = "NCVS_crime_rates_2"
)
