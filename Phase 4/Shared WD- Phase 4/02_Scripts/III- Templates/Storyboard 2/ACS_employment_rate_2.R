...
# ------------------------------------------------------------------------------
USER_SAMPLE <- "us2023c" 
# Added EMPSTAT to the core extraction list
...
if (!file.exists(TARGET_FILE)) {
  ...
  extract_def <- define_extract_micro(
    ...
    description = "Three Countries ACS Skyline - Employment Rate (V2)"
  )
...
prepared_data <- as_tibble(dt_raw) %>%
  ...
  mutate(
    ...
    # INDICATOR LOGIC: Employment-to-Population Ratio
    # In IPUMS, EMPSTAT == 1 means "Employed". 
  ...
  # UNIVERSE FILTER: Prime Working-Age Adults (25-64)
  # We exclude emp_num == 0 because that means N/A.
...
p <- plot_economic_skyline(
  ...
  # Outputs cleanly to ACS_employment_rate_2.png
  plot_title = "ACS_employment_rate_2"
) +
  labs(
    caption = stringr::str_wrap("Note: Universe restricted to prime working-age adults (25-64) to calculate a structural employment-to-population ratio. Income is generated by natively aggregating personal income (INCTOT) within households. Data is adjusted for inflation and spatial price parity.", width = 110)
  ) +
  theme(
