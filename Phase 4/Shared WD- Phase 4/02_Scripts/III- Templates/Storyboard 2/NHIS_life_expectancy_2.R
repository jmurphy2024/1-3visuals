# ==============================================================================
# SCRIPT: NHIS_life_expectancy_2.R
# Purpose: Generate 3-Country Skyline for Adult Life Expectancy (Age 25+)
# Logic:   Jittered NHIS Brackets + Mortality Linkage + Dynamic V2 Borders
# Note:    NHIS does not collect INCTOT; categorical jittering is required.
# ==============================================================================
rm(list = ls()); gc()
...
TARGET_DIR  <- here::here("01_data", "raw", "IPUMS_Microdata", "nhis_life_expectancy")
TARGET_FILE <- file.path(TARGET_DIR, "mortality_data_2.rds")
...
if (!file.exists(TARGET_FILE)) {
  ...
  extract_def <- define_extract_micro(
    ...
    description = "Three Countries Adult Life Expectancy (V2)"
  )
...
# 4. NORMALIZATION & JITTERING (Categorical Data)
# ------------------------------------------------------------------------------
...
prepared_data <- raw_data %>%
  ...
  mutate(
    ...
    # Jittering logic to convert categorical brackets to continuous numeric
  ...
  filter(!is.na(REAL_INCOME), !is.na(Country), PERWT > 0)

# 5. DEMOGRAPHIC SUMMARIZATION (Decile Smoothing & Life Tables)
# ------------------------------------------------------------------------------
# Preserved exact demographic life table function
...
# 6. VISUALIZATION EXECUTION (V2 Standard)
# ------------------------------------------------------------------------------
# DYNAMIC V2 Axis Labels
...
p <- ggplot(viz_data, aes(x = x_id, y = Life_Expectancy)) +
  ...
  labs(
    ...
    #title = "NHIS_life_expectancy_2",
    caption = stringr::str_wrap("Note: Displays Adult Life Expectancy (calculated at age 25). Uses IPUMS NHIS 2018 demographic life tables with categorical income uniformly distributed prior to RPP and inflation adjustments. The unusually high survival rate at the absolute bottom represents the 'Zero-Income Anomaly.", width = 110)
  )

print(p)

ggsave(
  filename = here::here("03_output", "visualizations_final", "NHIS_life_expectancy_2.png"), 
  plot = p, 
  width = 10, 
  height = 6.5, 
  dpi = 300
