# ==============================================================================
# SCRIPT: GSS_mode_of_transportation_2C.R
# Purpose: Generate 3-Country Skyline for Commuter Mobility & Transit Readiness
...
prepared_data <- raw_data %>%
  ...
  mutate(
    ...
    # PILLAR LOGIC
    `Commuter Necessity` = if_else(!is.na(WRKSTAT) & WRKSTAT <= 2, 1, 0, missing = 0), 
    `Pedestrian Safety`  = if_else(!is.na(FEAR) & FEAR == 2, 1, 0, missing = 0),
    `Physical Capacity`  = if_else(!is.na(HEALTH) & HEALTH <= 2, 1, 0, missing = 0),
    Mobility_Index       = if_else(`Commuter Necessity` == 1 & `Pedestrian Safety` == 1 & `Physical Capacity` == 1, 1, 0),
...
# ------------------------------------------------------------------------------
message("\n=== COMMUTER MOBILITY SUMMARY ===")
overall_tercile <- prepared_data %>%
  group_by(Country) %>%
  summarise(
    Subgroup = "Overall Tercile", Total_Population = sum(PERWT, na.rm = TRUE),
    `Commuter Necessity (%)` = round((sum(`Commuter Necessity` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Pedestrian Safety (%)`  = round((sum(`Pedestrian Safety` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Physical Capacity (%)`  = round((sum(`Physical Capacity` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Mobility Index (%)`     = round((sum(Mobility_Index * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    `Commuter Necessity` = weighted.mean(`Commuter Necessity`, w = PERWT, na.rm = TRUE),
    `Pedestrian Safety`  = weighted.mean(`Pedestrian Safety`, w = PERWT, na.rm = TRUE),
    `Physical Capacity`  = weighted.mean(`Physical Capacity`, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c("Commuter Necessity", "Pedestrian Safety", "Physical Capacity"), names_to = "Variable", values_to = "val") %>%
  mutate(Country = factor(Country, levels = c("Bottom Third", "Middle Third", "Top Third"))) %>%
...
viz_data_single <- prepared_data %>%
  group_by(Country) %>% mutate(decile = ntile(REAL_INCOME, 10)) %>% group_by(Country, decile) %>%
  summarise(avg_val = weighted.mean(Mobility_Index, w = PERWT, na.rm = TRUE), .groups = "drop") %>%
...
ggsave(here::here("03_output", "visualizations_final", "GSS_transit_mobility_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "GSS_transit_mobility_pillars_2C.png"), p_pillars, width = 10, height = 7, dpi = 300, bg = "white")
ggsave(here::here("03_output", "visualizations_final", "GSS_transit_mobility_composite_2C.png"), p_index, width = 10, height = 7, dpi = 300, bg = "white")
