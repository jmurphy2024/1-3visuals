...
# Purpose: Granular 10-decile focus on Child Poverty Intensity (Red Country).
# Logic:   Isolates children (Age < 18) and maps Income-to-Poverty Ratio.
# Branding: Master Red (#9B2226) with shadow effect (#FDEDEC).
# ==============================================================================
...
# Isolating Red Country children and calculating 10 internal deciles
red_child_intensity <- data %>%
  filter(
    as.numeric(AGE) < 18, 
    REAL_INCOME <= cutoffs$main_cutoff1
  ) %>%
  # Recalculate 10 deciles specifically for this population segment
...
p_red_child_int <- ggplot(red_child_intensity, aes(x = avg_income, y = avg_poverty_intensity)) +
  ...
  theme(
    axis.line       = element_line(color = "black", linewidth = 1.5), # Professional weight
