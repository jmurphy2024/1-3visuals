...
# ------------------------------------------------------------------------------
USER_SAMPLE <- "us2023c" # ACS 5-Year Sample

# Swapped FTOTINC for SERIAL and INCTOT to enable ground-up aggregation
VARS_NEEDED <- c("SERIAL", "PERWT", "INCTOT", "EDUC", "STATEFIP", "AGE")

TARGET_DIR  <- here::here("01_data", "raw", "IPUMS_Microdata", "acs_education")
TARGET_FILE <- file.path(TARGET_DIR, "acs_education_aggregated.rds")
...
if (!file.exists(TARGET_FILE)) {
  ...
  extract_def <- define_extract_micro(
    ...
    description = "Three Countries ACS Educational Attainment (BA+) - V2 Aggregation"
  )
...
# ------------------------------------------------------------------------------
# Sum individual income (INCTOT) for all people living under the same roof
hh_aggregated <- raw_data %>%
  rename_with(toupper, everything()) %>%
  filter(INCTOT != 9999999) %>%
  group_by(SERIAL) %>%
  summarise(AGGREGATED_HHINCOME = sum(INCTOT, na.rm = TRUE), .groups = "drop")
...
prepared_data <- raw_data %>%
  ...
  mutate(
    ...
    # Run the true aggregate income through RPP and Inflation
    REAL_INCOME = (AGGREGATED_HHINCOME * INFLATION_ADJ) * (100 / coalesce(STATE_RPP, 100)),
    
    # Map to V2 Dynamic Borders
    Country = case_when(
  ...
  # UNIVERSE: Restrict to Age 25+ for valid education tracking
  filter(!is.na(PERWT), PERWT > 0, as.numeric(AGE) >= 25, !is.na(REAL_INCOME), !is.na(Country))
...
p <- plot_economic_skyline(
  ...
  plot_title = "ACS_Education_Skyline_V2"
) +
  labs(
    caption = stringr::str_wrap("Note: Universe restricted to adults aged 25 and older. Income is generated by natively aggregating personal income (INCTOT) within households to bypass Census top-coding and imputation variance. Data is adjusted for inflation and spatial price parity.", width = 110)
  ) +
  theme(
