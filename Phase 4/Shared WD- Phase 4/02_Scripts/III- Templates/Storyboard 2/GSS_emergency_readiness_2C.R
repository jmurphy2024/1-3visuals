# ==============================================================================
# SCRIPT: GSS_emergency_readiness_2C.R
...
overall_tercile <- prepared_data %>%
  ...
  summarise(
    ...
    `Physical Readiness (%)`      = round((sum(`Physical Readiness` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Social Readiness (%)`        = round((sum(`Social Readiness` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Institutional Readiness (%)` = round((sum(`Institutional Readiness` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Readiness Index (%)`         = round((sum(Emergency_Readiness_Index * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    `Physical Readiness`      = weighted.mean(`Physical Readiness`, w = PERWT, na.rm = TRUE),
    `Social Readiness`        = weighted.mean(`Social Readiness`, w = PERWT, na.rm = TRUE),
    `Institutional Readiness` = weighted.mean(`Institutional Readiness`, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c("Physical Readiness", "Social Readiness", "Institutional Readiness"), 
...
viz_data_single <- prepared_data %>%
  ...
  mutate(decile = ntile(REAL_INCOME, 10)) %>%
  group_by(Country, decile) %>%
  summarise(avg_val = weighted.mean(Emergency_Readiness_Index, w = PERWT, na.rm = TRUE), .groups = "drop") %>%
...
ggsave(here::here("03_output", "visualizations_final", "GSS_emergency_readiness_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "GSS_emergency_readiness_pillars_2C.png"), p_pillars, width = 10, height = 7, dpi = 300, bg = "white")
ggsave(here::here("03_output", "visualizations_final", "GSS_emergency_readiness_composite_2C.png"), p_index, width = 10, height = 7, dpi = 300, bg = "white")
