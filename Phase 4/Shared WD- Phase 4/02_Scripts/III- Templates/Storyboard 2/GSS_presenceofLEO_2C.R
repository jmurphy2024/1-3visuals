...
prepared_data <- raw_data %>%
  ...
  mutate(
    ...
    LEO_Attitude_Index    = if_else(`Prevalence Beliefs` == 1 & `Utilization Beliefs` == 1 & `Systemic Attitude` == 1, 1, 0),
...
# ------------------------------------------------------------------------------
message("\n=== LEO ATTITUDES SUMMARY ===")
overall_tercile <- prepared_data %>%
  group_by(Country) %>%
  summarise(
    Subgroup = "Overall Tercile", Total_Population = sum(PERWT, na.rm = TRUE),
    `Prevalence Beliefs (%)`  = round((sum(`Prevalence Beliefs` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Utilization Beliefs (%)` = round((sum(`Utilization Beliefs` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Systemic Attitude (%)`   = round((sum(`Systemic Attitude` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `LEO Attitude Index (%)`  = round((sum(LEO_Attitude_Index * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    `Prevalence Beliefs`  = weighted.mean(`Prevalence Beliefs`, w = PERWT, na.rm = TRUE),
    `Utilization Beliefs` = weighted.mean(`Utilization Beliefs`, w = PERWT, na.rm = TRUE),
    `Systemic Attitude`   = weighted.mean(`Systemic Attitude`, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c("Prevalence Beliefs", "Utilization Beliefs", "Systemic Attitude"), names_to = "Variable", values_to = "val") %>%
  mutate(Country = factor(Country, levels = c("Bottom Third", "Middle Third", "Top Third"))) %>%
...
viz_data_single <- prepared_data %>%
  group_by(Country) %>% mutate(decile = ntile(REAL_INCOME, 10)) %>% group_by(Country, decile) %>%
  summarise(avg_val = weighted.mean(LEO_Attitude_Index, w = PERWT, na.rm = TRUE), .groups = "drop") %>%
...
ggsave(here::here("03_output", "visualizations_final", "GSS_LEO_attitudes_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "GSS_LEO_attitudes_pillars_2C.png"), p_pillars, width = 10, height = 7, dpi = 300, bg = "white")
ggsave(here::here("03_output", "visualizations_final", "GSS_LEO_attitudes_composite_2C.png"), p_index, width = 10, height = 7, dpi = 300, bg = "white")
