# ==============================================================================
# SCRIPT: ACS_health_insurance_type_2.R
# Purpose: Generate 3-Country Skyline for Public vs Private Health Insurance
...
# ------------------------------------------------------------------------------
# Source the V2 Normalization script which contains plot_economic_skyline_2()
...
# Target Variables: HCOVPRIV (Private coverage) & HCOVPUB (Public coverage)
VARS_NEEDED <- c("SERIAL", "PERWT", "INCTOT", "STATEFIP", "AGE", "HCOVPRIV", "HCOVPUB")

TARGET_DIR  <- here::here("01_data", "raw", "IPUMS_Microdata", "acs_health_insurance")
TARGET_FILE <- file.path(TARGET_DIR, "acs_health_insurance_type_aggregated_2.rds")
...
if (!file.exists(TARGET_FILE)) {
  ...
  extract_def <- define_extract_micro(
    ...
    description = "Three Countries ACS Skyline - Public vs Private Insurance (V2)"
  )
...
prepared_data <- as_tibble(dt_raw) %>%
  ...
  mutate(
    ...
    # --------------------------------------------------------------------------
    # INDICATOR LOGIC: Public vs Private Insurance
    # (Shortened names to "Private" and "Public" to fit the margins)
    # --------------------------------------------------------------------------
    Private = if_else(as.numeric(HCOVPRIV) == 2, 1, 0),
    Public  = if_else(as.numeric(HCOVPUB) == 2, 1, 0),
  ...
  filter(!is.na(PERWT), PERWT > 0, !is.na(REAL_INCOME), !is.na(Country), 
         as.numeric(HCOVPRIV) > 0, as.numeric(HCOVPUB) > 0)

# 5. VISUALIZATION
# ------------------------------------------------------------------------------
p <- plot_economic_skyline_2(
  data           = prepared_data, 
  indicator_vars = c("Private", "Public"), # Updated to match shortened names
  weight_var     = "PERWT", 
  y_axis_label   = "Health Insurance Coverage Rate (%)",
  plot_title     = "ACS_health_insurance_type_2"
) +
  labs(
    caption = stringr::str_wrap("Note: Universe includes all persons with valid health insurance responses. Income is generated by natively aggregating personal income (INCTOT) within households. Data is adjusted for inflation and spatial price parity.", width = 110)
  ) +
  theme(
    # INCREASED RIGHT MARGIN (r = 80) to prevent text cutoff
    plot.margin = margin(t = 20, r = 80, b = 50, l = 10), 
