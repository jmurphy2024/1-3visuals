# ==============================================================================
# SCRIPT: ACS_employment_rate_2D.R
...
overall_tercile <- prepared_data %>%
  ...
  summarise(
    ...
    `Employed (%)` = round((sum(target_indicator * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
ggsave(here::here("03_output", "visualizations_final", "ACS_employment_Summary_Table_2D.png"), table_grob, width = 10, height = 6, bg = "white", dpi = 300)

# 6. VISUALIZATION EXECUTION (Minimalist 2D Format)
# ------------------------------------------------------------------------------
message("\nGenerating Minimalist Skyline Plot...")

p <- plot_economic_skyline_2C(
  data          = prepared_data, 
  indicator_var = "target_indicator", 
  weight_var    = "PERWT"
)

ggsave(here::here("03_output", "visualizations_final", "ACS_employment_rate_2D.png"), p, width = 10, height = 7, dpi = 300, bg = "white")
