...
# Logic: Uses max() to create binary victim flags + Dynamic V2 Borders
# ==============================================================================
rm(list = ls()); gc()
...
# 5. THE UNIFIED PIPELINE (PREVALENCE LOGIC)
prepared_data <- ds3_unique %>%
  ...
  summarise(
    ...
    # PREVALENCE: Uses max() to capture IF it happened at least once
...
# 6. VISUALIZATION (With Prevalence Caption)
# ------------------------------------------------------------------------------
plot_data <- prepared_data %>% rename(`Property Crime` = Property_Crime, `Violent Crime` = Violent_Crime)

prevalence_note <- paste0(
  "Prevalence Rate: Measures the percentage of the population victimized at least once this year.\n",
  "Note: This metric does not count repeat victimizations against the same individual.\n."
