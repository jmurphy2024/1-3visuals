# ==============================================================================
# SCRIPT: NCVS_Combined_Victimization_Skyline.R
# Purpose: Generate 3-Country Skyline for Both Violent & Property Victimization
...
# 6. VISUALIZATION: MULTI-LINE SKYLINE WITH DIRECT LABELS
# Summarize both indicators and pivot them for ggplot
viz_data <- prepared_data %>%
  group_by(Country) %>%
  mutate(ventile = ntile(REAL_INCOME, 20)) %>% 
  group_by(Country, ventile) %>%
  summarise(
    `Violent Crime`  = weighted.mean(ind_violent, w = PERWT, na.rm = TRUE),
    `Property Crime` = weighted.mean(ind_property, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
# Map axis formatting from your II-D standards
...
p <- ggplot(viz_data, aes(x = x_id, y = Rate, color = Country)) +
  # Draw solid lines for both, grouped so they don't break
  ...
  # DIRECT LABELING: Place the name of the crime at the end of each line
  ...
  # Expand the right side of the x-axis specifically to make room for the text labels
  scale_x_continuous(breaks = income_breaks, labels = income_labels, expand = expansion(mult = c(0.01, 0.18))) + 
  theme_minimal() + 
  theme(
    legend.position = "none", # Legend removed since lines are labeled directly
  ...
  labs(x = "Household Income (Real Adjusted Dollars)", y = "Victimization Rate (%)")

# Auto-Save to Visualizations Final Folder
out_dir <- here::here("03_output", "visualizations_final")
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

ggsave(file.path(out_dir, "NCVS_2023_Combined_Crime_Skyline.png"), p, width = 10, height = 6, dpi = 300)
print(p)