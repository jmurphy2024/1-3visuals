...
prepared_data <- ds3_unique %>%
  ...
  summarise(
    ...
    Violent_Crime = sum(Violent_Crime, na.rm = TRUE), # Summing for Incidence Count
...
# ------------------------------------------------------------------------------
message("\n=== NCVS INCIDENCE SUMMARY (Per 100 Persons) ===")
overall_tercile <- prepared_data %>%
  group_by(Country) %>%
  summarise(
    Subgroup = "Overall Tercile", Total_Population = sum(PERWT, na.rm = TRUE),
    `Property Crime (per 100)` = round((sum(`Property Crime` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Violent Crime (per 100)`  = round((sum(`Violent Crime` * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    # Multiply mean by 100 to convert to a "Per 100 Persons" rate natively
    `Property Crime` = weighted.mean(`Property Crime`, w = PERWT, na.rm = TRUE) * 100,
    `Violent Crime`  = weighted.mean(`Violent Crime`, w = PERWT, na.rm = TRUE) * 100,
    .groups = "drop"
  ) %>%
...
p_chart <- ggplot(viz_data_multi, aes(x = x_id, y = val, group = Variable)) +
  ...
  scale_y_continuous(labels = scales::label_number(accuracy = 0.1), limits = c(0, NA), breaks = function(x) c(0, max(x, na.rm = TRUE)), expand = expansion(mult = c(0, 0.05))) +
...
ggsave(here::here("03_output", "visualizations_final", "NCVS_Incidence_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "NCVS_Incidence_Skyline_2C.png"), p_chart, width = 10, height = 7, dpi = 300, bg = "white")
