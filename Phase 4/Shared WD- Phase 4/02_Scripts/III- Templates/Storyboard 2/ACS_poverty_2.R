...
if (!file.exists(TARGET_FILE)) {
  ...
  extract_def <- define_extract_micro(
    ...
    description = "Three Countries ACS Skyline - Poverty Ratio Bottom Third (V2)"
  )
...
prepared_data <- as_tibble(dt_raw) %>%
  ...
  mutate(
    ...
    # INDICATOR: Income-to-Poverty Ratio (e.g., 50% of poverty line, 150%, etc.)
    # Dividing by 100 allows plot_economic_skyline's y_format="percent" to display it perfectly.
  ...
  # UNIVERSE FILTER: Restrict strictly to the Bottom Third + Prime Working Age
  filter(!is.na(PERWT), PERWT > 0, !is.na(REAL_INCOME), 
         Country == "Bottom Third",  # ISOLATING THE RED COUNTRY
...
p <- plot_economic_skyline(
  ...
  labs(
    caption = stringr::str_wrap("Note: Universe restricted to adults aged 25-64 in the Bottom Third of the income distribution. The indicator maps the average income relative to the official Federal Poverty Line. Income is generated by natively aggregating personal income (INCTOT) within households.", width = 110)
  ) +
  theme(
