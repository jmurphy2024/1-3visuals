...
overall_tercile <- prepared_data %>%
  ...
  summarise(
    ...
    `Opportunities (%)` = round((sum(Opportunities * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Resources (%)`     = round((sum(Resources * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Outcomes (%)`      = round((sum(Outcomes * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Equity Index (%)`  = round((sum(Equity_Index * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    Opportunities = weighted.mean(Opportunities, w = PERWT, na.rm = TRUE),
    Resources     = weighted.mean(Resources, w = PERWT, na.rm = TRUE),
    Outcomes      = weighted.mean(Outcomes, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c("Opportunities", "Resources", "Outcomes"), names_to = "Variable", values_to = "val") %>%
  mutate(Country = factor(Country, levels = c("Bottom Third", "Middle Third", "Top Third"))) %>%
...
viz_data_single <- prepared_data %>%
  group_by(Country) %>% mutate(decile = ntile(REAL_INCOME, 10)) %>% group_by(Country, decile) %>%
  summarise(avg_val = weighted.mean(Equity_Index, w = PERWT, na.rm = TRUE), .groups = "drop") %>%
...
ggsave(here::here("03_output", "visualizations_final", "GSS_equity_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "GSS_equity_pillars_2C.png"), p_pillars, width = 10, height = 7, dpi = 300, bg = "white")
ggsave(here::here("03_output", "visualizations_final", "GSS_equity_composite_2C.png"), p_index, width = 10, height = 7, dpi = 300, bg = "white")
