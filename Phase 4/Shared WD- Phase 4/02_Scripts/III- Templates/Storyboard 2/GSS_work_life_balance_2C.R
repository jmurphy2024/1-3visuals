# ==============================================================================
# SCRIPT: GSS_work_life_balance_2C.R
...
overall_tercile <- prepared_data %>%
  ...
  summarise(
    ...
    `Sovereignty (%)` = round((sum(ind_sovereignty * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Support (%)`     = round((sum(ind_support * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `Fulfillment (%)` = round((sum(ind_fulfillment * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    `WLB Index (%)`   = round((sum(ind_wlb_index * PERWT, na.rm = TRUE) / Total_Population) * 100, 1),
    .groups = "drop"
  )
...
viz_data_multi <- prepared_data %>%
  ...
  summarise(
    Sovereignty = weighted.mean(ind_sovereignty, w = PERWT, na.rm = TRUE),
    Support     = weighted.mean(ind_support, w = PERWT, na.rm = TRUE),
    Fulfillment = weighted.mean(ind_fulfillment, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c("Sovereignty", "Support", "Fulfillment"), names_to = "Variable", values_to = "val") %>%
  mutate(Country = factor(Country, levels = c("Bottom Third", "Middle Third", "Top Third"))) %>%
...
viz_data_single <- prepared_data %>%
  group_by(Country) %>% mutate(decile = ntile(REAL_INCOME, 10)) %>% group_by(Country, decile) %>%
  summarise(avg_val = weighted.mean(ind_wlb_index, w = PERWT, na.rm = TRUE), .groups = "drop") %>%
...
ggsave(here::here("03_output", "visualizations_final", "GSS_work_life_balance_Summary_Table_2C.png"), table_grob, width = 10, height = 5, bg = "white", dpi = 300)
ggsave(here::here("03_output", "visualizations_final", "GSS_work_life_balance_pillars_2C.png"), p_pillars, width = 10, height = 7, dpi = 300, bg = "white")
ggsave(here::here("03_output", "visualizations_final", "GSS_work_life_balance_composite_2C.png"), p_index, width = 10, height = 7, dpi = 300, bg = "white")
