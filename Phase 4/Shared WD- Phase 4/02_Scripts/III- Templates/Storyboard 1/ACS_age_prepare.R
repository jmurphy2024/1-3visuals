...
# Purpose: Prepares inclusive age data.
# Logic: No PERNUM filter; weights set to PERWT.
# ==============================================================================
...
if(!file.exists(raw_file)) stop("Raw data not found.")
...
# --- 3. TRANSFORM ---
message("Processing age-inclusive population...")

prepared_age_data <- raw_data %>%
  # Keep everyone in the household
  mutate(
    AGE      = as.numeric(AGE),
    STATEFIP = as.numeric(STATEFIP),
    #shared HH income logic
    income_raw = as.numeric(HHINCOME),
    income_cleaned = if_else(income_raw >= 9999999, NA_real_, income_raw),
...
# Save for Age visualization
output_file <- here::here("01_data", "processed", "prepared_ACS_inclusive_age.rds")
saveRDS(prepared_age_data, output_file)

message(paste("Age Prep Complete. Total population:", format(sum(prepared_age_data$PERWT), big.mark=",")))