...
# Script: ACS_race_Prepare.R
# Purpose: Calculates Real Income and creates 'Race_Ethnicity' categories.
# Output:  01_data/processed/IPUMS_Microdata/prepared_ACS_Race_us2023b.rds
# ==============================================================================

rm(list = ls()); gc()
library(dplyr); library(readr); library(here); library(tibble)

# --- 1. SETUP ---
SAMPLE_ID <- "us2023c"
RAW_FILE  <- here::here("01_data", "raw", "IPUMS_Microdata", paste0("usa_", SAMPLE_ID, "_race"), "raw_data.rds")

if(!file.exists(RAW_FILE)) stop("Raw Race file not found. Run Script 1.")
...
message("Processing Race & Income Data...")
...
prepared_data <- raw_data %>%
  ...
  mutate(
    ...
    # 1. Start with Hispanic (Overrides Race)
    # 2. Then Non-Hispanic categories
...
saveRDS(prepared_data, file.path(out_dir, "prepared_ACS_Race_us2023c.rds"))
message("Success.")