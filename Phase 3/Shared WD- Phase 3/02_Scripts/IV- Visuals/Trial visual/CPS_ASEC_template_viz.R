...
## Script: CPS_ASEC_template_viz.R
## Purpose: Visualizes a prepared CPS ASEC indicator and generates academic summary tables.
...
## Dependencies: All plotting libraries, dplyr, readr, here, rlang.
## Input: A processed RDS file from the corresponding 'prepare' script.

# ==== 0. SETUP ====
rm(list = ls()); gc()

# Check for and install Hmisc if missing (Required for weighted tables)
...
PLOT_TYPE         <- "multi"
Y_VARS            <- c("unemployment_rate_u3", "unemployment_rate_u6") 
Y_LABELS          <- c("U-3 Rate", "U-6 Rate") 

# --- 1.5. Define Table Parameters ---
TABLE_VAR         <- "unemployment_rate_u3" 
TABLE_VAR_LABEL   <- "Unemployment Rate" 

PLOT_TITLE        <- "Unemployment Rates (U-3 & U-6) by Position (ASEC)"
Y_AXIS_LABEL      <- "Unemployment Rate (Age 25+)"
...
OUTPUT_PLOT_FILENAME <- paste0("plot_", USER_INDICATOR_NAME, "_", USER_ASEC_SAMPLE_ID, ".png")
...
# Save Outputs to CSVs/PNGs Folders
...
png_filename <- paste0("summary_table_detailed_CPS_", USER_INDICATOR_NAME, ".png")
