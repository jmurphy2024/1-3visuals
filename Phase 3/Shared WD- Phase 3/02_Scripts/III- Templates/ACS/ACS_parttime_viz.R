...
## Script: ACS_nonstandard_job_viz.R
## Purpose: Generates Prevalence-based line plot and summary table for Non-Standard Jobs.
...
OUTPUT_PLOT_NAME  <- "plot_nonstandard_job.png"
OUTPUT_TABLE_NAME <- "table_nonstandard_job.png"
...
total_pop <- data_with_groups %>%
  summarise(
    ...
    n = n(),
    "Non-Standard Job" = weighted.mean(ind_nonstandard, w = .data[[USER_WEIGHT_VARIABLE]], na.rm = TRUE)
  )
...
country_summary <- data_with_groups %>%
  ...
  select(Country, n, "Non-Standard Job")

# Formatting for tableGrob (Matches template design)
table_data_final <- bind_rows(total_pop, country_summary) %>%
  mutate(across(where(is.numeric) & !n, ~sprintf("%.1f%%", . * 100))) %>%
  mutate(n = scales::comma(n))

colnames(table_data_final) <- c("bold('Country')", "bold(italic('n'))", "bold('Part-time / Non-Standard')")
...
final_plot <- create_single_line_plot(
  ...
  y_var              = "Non-Standard Job",
  plot_title         = "Prevalence of Part-time or Non-Standard Jobs",
...
# Ensure percent format and display
final_plot <- final_plot + scale_y_continuous(labels = scales::percent_format(accuracy = 1))
...
notes_vbox <- grobTree(
  ...
  textGrob(paste0("1. Source: IPUMS ACS 2023"), 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.75),
  
  textGrob("2. Universe: Active workers age 16+ with reported usual hours worked. Denominator includes all workers.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.60),
  
  textGrob("3. Interpretation: Percentages represent the share of workers working fewer than 35 hours per week.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.45),
  
  textGrob("4. Non-Standard: Defined statistically as part-time status based on UHRSWORK variable.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
...
message("SUCCESS: Visuals exported and displayed in RStudio panel.")