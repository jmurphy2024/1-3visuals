...
## Script: ACS_transportation_viz.R
## Purpose: Generates Prevalence-based line plot and summary table for Private Transport.
...
USER_IPUMS_SAMPLE_ID  <- "us2023a" #B is for the 5-year sample
USER_INDICATOR_NAME   <- "privatetransportation"
USER_WEIGHT_VARIABLE  <- "PERWT"

OUTPUT_PLOT_NAME  <- "plot_transportation.png"
OUTPUT_TABLE_NAME <- "table_transportation.png"

# Load prepared data
PREPARED_DATA_FILE <- here::here("01_data", "processed", "IPUMS_Microdata", 
                                 paste0("prepared_ACS_Transportation_Mode_", USER_IPUMS_SAMPLE_ID, ".rds"))
...
total_pop <- data_with_groups %>%
  summarise(
    ...
    n = n(),
    "Private Transport" = weighted.mean(ind_private, w = .data[[USER_WEIGHT_VARIABLE]], na.rm = TRUE)
  )
...
country_summary <- data_with_groups %>%
  ...
  select(Country, n, "Private Transport")
...
colnames(table_data_final) <- c("bold('Country')", "bold(italic('n'))", "bold('Private Transport')")
...
final_plot <- create_single_line_plot(
  ...
  y_var              = "Private Transport",
  plot_title         = "Prevalence of Private Transportation to Work",
...
notes_vbox <- grobTree(
  ...
  textGrob("2. Universe: Workers age 16+ with known household income. Denominator includes all modes of transportation.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.60),
  
  textGrob("3. Interpretation: Percentages represent the share of workers using private vehicles.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.45),
  
  textGrob("4. Private Vehicle: Includes Cars, Trucks, Vans, or Motorcycles.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
...
# Add this after creating final_plot to force % labels
final_plot <- final_plot + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))

# Now display and save
grid.newpage(); grid.draw(final_plot)
