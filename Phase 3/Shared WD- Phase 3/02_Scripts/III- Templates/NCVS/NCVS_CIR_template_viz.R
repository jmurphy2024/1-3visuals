...
summary_plotting <- data_with_groups %>%
  ...
  summarise(
    # CHANGED: Calculate ONLY the Reporting Rate (ind_police derived from V4399)
    "Reported to Police" = weighted.mean(ind_police, w = PERWT, na.rm = TRUE),
    .groups = "drop"
  )

final_plot <- create_multi_line_plot(
  summary_data       = summary_plotting,
  # CHANGED: Select ONLY the reporting variable for the y-axis
  y_vars             = c("Reported to Police"),
  y_labels           = c("Reported to Police"),
  plot_title         = "Criminal Incident Reporting Rates",
...
notes_vbox <- grobTree(
  ...
  # CHANGED: Updated to reflect Reporting Rate definition
  textGrob("2. Percentages represent the weighted rate of crimes reported to police (V4399 = Yes).", gp = gpar(fontface = "italic", fontsize = 8.5, family = "serif"), hjust = 0, x = 0.05, y = 0.69),
  # CHANGED: Clarified the denominator
  textGrob("3. Denominator includes all persons; numerator is the count of reported incidents.", gp = gpar(fontface = "italic", fontsize = 8.5, family = "serif"), hjust = 0, x = 0.05, y = 0.56)
)
