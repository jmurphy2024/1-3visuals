...
## Script: NCVS_presenceLEO_viz.R
## Purpose: Generates Prevalence-based multi-line plot and summary table for LE Presence.
...
# Load the updated prevalence dataset
...
OUTPUT_PLOT_NAME  <- "plot_presenceofLEO.png"
OUTPUT_TABLE_NAME <- "table_presenceofLEO.png"
...
# ==== 2. DATA SUMMARIZATION (Reporting & Presence) ====
...
total_population <- data_with_groups %>%
  summarise(
    ...
    n = n(),
    "Reported"       = weighted.mean(ind_reported, w = PERWT, na.rm = TRUE),
    "Police Present" = weighted.mean(ind_police_present, w = PERWT, na.rm = TRUE)
  )

# 2.2. Country Summaries (FIXED SELECTION)
country_summary <- data_with_groups %>%
  ...
  # Select actual column names to avoid NA% results
  select(Country, n, "Reported", "Police Present")
...
# Rename headers for the tableGrob
colnames(table_data_final) <- c(
  "bold('Country')", 
  "bold(italic('n'))", 
  "bold('Crimes Reported')", 
  "bold('Police Present')"
)
...
final_plot <- create_multi_line_plot(
  ...
  y_vars           = c("Reported", "Police Present"),
  y_labels         = c("Crimes Reported to Police", "Police Physically Present"),
  plot_title       = "Law Enforcement Presence vs. Reporting",
  y_axis_label     = "Weighted Percentage of Population",
  y_axis_format    = "percent",
  y_limits         = c(0.0002, 0.001),
...
# FIXED: Vertical spacing to stop text overlap
notes_vbox <- grobTree(
  textGrob("Notes:", gp = gpar(fontface = "bold", fontsize = 8.5, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.90),
  textGrob("1. Data: 2023 NCVS Household (DS2), Person (DS3), and Incident (DS5) files.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.75),
  textGrob("2. Interpretation: Percentages reflect the prevalence of reporting crime and if police showed up.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.60),
  textGrob("3. Criteria: Based on victim-reported police notification and physical officer presence at the scene.", 
           gp = gpar(fontface = "italic", fontsize = 8, family = "serif"), 
           hjust = 0, x = 0.05, y = 0.45)
)

# Increase height of notes section to 8 lines
final_layout <- grid.arrange(table_grob, notes_vbox, heights = unit.c(unit(1, "null"), unit(8, "lines")))

# Save table
png(here::here("03_output", TABLE_SUBFOLDER, OUTPUT_TABLE_NAME), width = 900, height = 500, res = 120)
grid.draw(final_layout)
dev.off()

message("SUCCESS: Viz script complete. Check your 03_output folders.")