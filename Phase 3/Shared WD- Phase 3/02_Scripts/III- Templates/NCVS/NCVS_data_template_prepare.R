...
## Script: NCVS_data_template_prepare.R
...
prepared_data <- ds3_unique %>%
  ...
  left_join(ds5_raw %>% select(IDHH, IDPER, V4529, V4399), 
            by = c("IDHH", "IDPER"),
  ...
  mutate(
    ...
    police_code = extract_code(V4399),
    ...
    ind_police   = if_else(!is.na(police_code) & police_code == 1, 1, 0)
  ) %>%
...
group_by(IDHH, IDPER) %>%
  summarise(
    ...
    ind_police   = max(ind_police, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
filter(!is.na(income_code) & income_code < 99 & PERWT > 0) %>%
  ...
  select(PERWT, HHINCOME, ind_violent, ind_property, ind_police)
